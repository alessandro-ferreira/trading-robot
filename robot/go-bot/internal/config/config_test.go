...
func TestNewWithDefaults(t *testing.T) {
	...
	assert.Equal(t, []time.Duration{0, 5 * time.Second, 15 * time.Second, 30 * time.Second}, cfg.Server.CheckPendingPolicy)
	...
	assert.False(t, cfg.Cron.MarketDataCleanup.Enabled)
	assert.Equal(t, "0 0 0 * * *", cfg.Cron.MarketDataCleanup.Schedule)
	assert.Equal(t, 7, cfg.Cron.MarketDataCleanup.RetentionDays)
	assert.False(t, cfg.Cron.MarketDataCleanup.RunOnStartup)
...
func TestLoad(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		assert.Equal(t, []time.Duration{0, 10 * time.Second}, cfg.Server.CheckPendingPolicy)
		...
		assert.Equal(t, 3*time.Second, cfg.Database.LockTimeout)
		assert.Equal(t, 2*time.Minute, cfg.Database.IdleInTxSessionTimeout)
		assert.Equal(t, "info", cfg.Log.Level)
		...
		// Verify Cron Config
		assert.True(t, cfg.Cron.MarketDataCleanup.Enabled)
		assert.Equal(t, "0 0 6 * * *", cfg.Cron.MarketDataCleanup.Schedule)
		assert.Equal(t, 14, cfg.Cron.MarketDataCleanup.RetentionDays)
		assert.True(t, cfg.Cron.MarketDataCleanup.RunOnStartup)
