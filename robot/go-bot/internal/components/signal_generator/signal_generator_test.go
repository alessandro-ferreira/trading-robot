		...
		},
		{
			...
			strategyCfg: repository.StrategyPair{
				...
				Momentum: repository.StrategyMomentum{
					...
					StopLossPct:     0.2 * 0.01,
					ActivationPct:   sql.NullFloat64{Float64: 0.3 * 0.01, Valid: true},
					TrailingStopPct: sql.NullFloat64{Float64: 0.1 * 0.01, Valid: true},
				},
			},
			wantErr: false,
		},
...
func TestSignalGenerator_Warmup(t *testing.T) {
	...
	t.Run("success with momentum trailing", func(t *testing.T) {
		stratCfg := repository.StrategyPair{
			...
			Momentum: repository.StrategyMomentum{
				WindowSeconds: 60,
				Windows: []repository.MomentumWindow{
					{LookbackSeconds: 30, Threshold: 0.01 * 0.01},
				},
				StopLossPct:     0.2 * 0.01,
				ActivationPct:   sql.NullFloat64{Float64: 0.5 * 0.01, Valid: true},
				TrailingStopPct: sql.NullFloat64{Float64: 0.1 * 0.01, Valid: true},
			},
		}
		...
		history := []repository.MarketDataTick{
			...
			{TickUnixAt: 1010, Price: 50010.0},
			{TickUnixAt: 1020, Price: 50020.0},
		}

		err = sg.Warmup(history)
		assert.NoError(t, err)
		// Verify it can process next tick after warmup
		_, err = sg.GetSignal(50030.0, 1030)
		assert.NoError(t, err)
	})
	...
	t.Run("success with momentum profit", func(t *testing.T) {
		stratCfg := repository.StrategyPair{
			...
			Momentum: repository.StrategyMomentum{
				WindowSeconds:   60,
				Windows:         []repository.MomentumWindow{{LookbackSeconds: 30, Threshold: 0.01 * 0.01}},
				StopLossPct:     0.2 * 0.01,
				ProfitTargetPct: sql.NullFloat64{Float64: 0.5 * 0.01, Valid: true},
			},
		}
	...
	t.Run("error with unsorted history", func(t *testing.T) {
		stratCfg := repository.StrategyPair{
			...
			Momentum: repository.StrategyMomentum{
				WindowSeconds:   60,
				Windows:         []repository.MomentumWindow{{LookbackSeconds: 30, Threshold: 0.1 * 0.01}},
				StopLossPct:     0.2 * 0.01,
				ProfitTargetPct: sql.NullFloat64{Float64: 0.5 * 0.01, Valid: true},
			},
		}
		...
		history := []repository.MarketDataTick{{TickUnixAt: 200, Price: 10.0}, {TickUnixAt: 100, Price: 10.1}}
...
func TestSignalGenerator_SetInPosition(t *testing.T) {
	stratCfg := repository.StrategyPair{
		...
		Momentum: repository.StrategyMomentum{
			WindowSeconds: 10,
			Windows: []repository.MomentumWindow{
				{LookbackSeconds: 5, Threshold: 0.1 * 0.01},
			},
			StopLossPct:     0.1 * 0.01,
			ActivationPct:   sql.NullFloat64{Float64: 0.5 * 0.01, Valid: true},
			TrailingStopPct: sql.NullFloat64{Float64: 0.2 * 0.01, Valid: true},
		},
	}
	...
	sg.SetInPosition(true, 50000.0, 50100.0)
...
func TestSignalGenerator_GetSignal(t *testing.T) {
	...
	t.Run("hold signal on small price move", func(t *testing.T) {
		// Threshold is 0.1%, so 50000 -> 50010 is < 0.1%
		_, err := sg.GetSignal(50000.0, 1000)
		require.NoError(t, err)

		sig, err := sg.GetSignal(50010.0, 1001)
		assert.NoError(t, err)
