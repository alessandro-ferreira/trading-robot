...
func TestService_Integration_GetBalance(t *testing.T) {
	...
	expectedResp, err := svc.client.GetBalance(ctx, assetSymbol, exchangeName)
...
func TestService_Integration_OrderLifecycle(t *testing.T) {
	...
	openOrders, err := svc.GetOpenOrders(ctx, symbol, exchangeName)
	...
	price1 := 40000.0
	amount1 := 0.1
	order1, err := svc.CreateOrder(ctx, symbol, repository.OrderSideBuy, repository.OrderTypeLimit, amount1, price1, exchangeName)
	require.NoError(t, err)
	...
	fetchedOrder1, err := svc.GetOrder(ctx, order1.Id, symbol, exchangeName)
	require.NoError(t, err)
	...
	price2 := 41000.0
	amount2 := 0.2
	order2, err := svc.CreateOrder(ctx, symbol, repository.OrderSideSell, repository.OrderTypeLimit, amount2, price2, exchangeName)
	require.NoError(t, err)
	...
	err = svc.CancelOrder(ctx, order2.Id, symbol, exchangeName)
	require.NoError(t, err)
