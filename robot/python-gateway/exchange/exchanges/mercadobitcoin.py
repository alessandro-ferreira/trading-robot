...
from .base import Exchange, Ticker, ExchangeError, OrderType, OrderSide
...
class MercadoBitcoinExchange(Exchange):
    ...
    def _request(self, method: str, path: str, data: Optional[Dict[str, Any]] = None) -> Any:
        ...
        try:
            if method == 'GET':
                response = requests.request(method, url, headers=headers, params=data, timeout=self.TIMEOUT)
            else:
    ...
    def cancel_order(self, id: str, symbol: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        if not symbol:
            raise ExchangeError("Symbol is required to cancel an order on Mercado Bitcoin")
    ...
    def fetch_order(self, id: str, symbol: Optional[str] = None) -> Dict[str, Any]:
        ...
        """
        if not symbol:
            raise ExchangeError("Symbol is required to fetch an order on Mercado Bitcoin")
        ...
        timestamp = int(response.get('created_at')) * 1000 if response.get('created_at') else None
        dt = datetime.fromtimestamp(timestamp / 1000, timezone.utc).isoformat() if timestamp else None
        ...
        return {
            ...
            'price': float(response.get('limitPrice')) if response.get('limitPrice') else None,
            'average': float(response.get('avgPrice')) if response.get('avgPrice') else None,
            'amount': float(response.get('qty')) if response.get('qty') else 0.0,
            'filled': float(response.get('filledQty')) if response.get('filledQty') else 0.0,
            'remaining': (float(response.get('qty')) - float(response.get('filledQty'))) if response.get('qty') and response.get('filledQty') else 0.0,
    ...
    def fetch_open_orders(self, symbol: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        for order in orders_data:
            ...
            timestamp = int(order.get('created_at')) * 1000 if order.get('created_at') else None
            dt = datetime.fromtimestamp(timestamp / 1000, timezone.utc).isoformat() if timestamp else None
            ...
            order_symbol = symbol if symbol else (order.get('instrument', '').replace('-', '/') if order.get('instrument') else None)
