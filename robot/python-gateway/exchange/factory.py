...
class ExchangeFactory:
    ...
    def _create_instance(self, name: str) -> Exchange:
        ...
        if not cfg:
            raise ExchangeNotConfigured(f"Exchange '{name}' is not configured or unknown")
        ...
        if cfg.sandbox_mode:
            try:
                ...
                raise ExchangeConfigurationError(f"Sandbox mode not supported or failed for {cfg.name}: {e}")
    ...
    def get(self, name: str) -> Exchange:
        ...
        if name not in self._instances:
            ...
            except (ExchangeNotConfigured, ExchangeConfigurationError) as e:
                raise
