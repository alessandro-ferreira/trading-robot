...
- [Getting Started](#getting-started)
  ...
  - [3. Usage](#3-usage)
  - [4. Comparison with Go Bot Backtesting](#4-comparison-with-go-bot-backtesting)
...
1767225600,2026-01-01 00:00,7176.8067,1719853200,2024-07-01 17:00,8253.8100,0.1501,profit_target
...
1767225600,2026-01-01 00:00,7174.3900,1782860400,2026-06-30 23:00,58624.7100,18.4048,accumulated
```

### 4. Comparison with Go Bot Backtesting

The simulator output should be compared against the `go-bot` backtest to validate that both implementations produce identical results.

To run an equivalent `go-bot` backtest:

1. Use the same parameters passed to `./build/simulator` in `robot/config.backtest.toml`, including `symbol`, `begin`, `end`, and `input`.
2. Set `orchestrator_interval` to a very small value, such as `1ms`, and use credentials to a test database.
3. Update the hard-coded strategy parameters in `robot/go-bot/internal/simulation/simulated_client.go` to match the strategy being tested.
4. Run the C++ simulator and Go backtest and compare their CSV outputs.

For example, the `config.backtest.toml` should contain:
...
input = "../../simulator/prices/XLM_prices.csv"
output = "/tmp/backtest_trades_XLM.csv"
initial_usdt = 10000.0
```

The strategy parameters are defined in `robot/go-bot/internal/simulation/simulated_client.go` and should match the corresponding simulator command-line arguments.

The output generated by:

```bash
./build/simulator ... --output /tmp/simulator_trades_XLM.csv
```

and the output configured through:

```toml
output = "/tmp/backtest_trades_XLM.csv"
```

in `robot/config.backtest.toml` should be **identical**.
